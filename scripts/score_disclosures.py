...
EXCEPT scores that already have an outcome_tracking row — those are closed
backtest records and are left untouched (see scoring/repository.py's
delete_scores_for_weight_set docstring, spec §10.2). Disclosures whose
score survived for that reason are skipped rather than re-scored, so this
never produces a second score row for the same disclosure+weight_set.
...
from stock_radar.backtest.repository import delete_rescoreable_scores_for_weight_set  # noqa: E402
from stock_radar.db.connection import get_connection  # noqa: E402
from stock_radar.scoring.repository import has_score_for_weight_set, save_score  # noqa: E402
...
def main() -> None:
    ...
    print(f"Scored {sum(rank_counts.values())} disclosure(s) under weight_set_id={weight_set_id}:")
    for rank in ("S", "A", "B", "none"):
        print(f"  {rank}: {rank_counts.get(rank, 0)}")
    if skipped_closed:
        print(f"  skipped {skipped_closed} disclosure(s) with a closed (outcome-tracked) score")
